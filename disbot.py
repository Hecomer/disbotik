...
class Disociate(discord.Client):
    ...
    async def on_message(self, message):
        ...
        if message.content.startswith("!таймер"):
            ...
            await message.channel.send(f'{str(client.get_emoji(748200364351815852))} Таймер сработает через {hours} hours и {minutes} minutes')
            await asyncio.sleep(hours * 3600 + minutes * 60)
            await message.channel.send(
                f"Ну как там с часами {str(client.get_emoji(748200275600212028))}")
